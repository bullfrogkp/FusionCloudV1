<cfscript>
	...
	if(NOT DirectoryExists("#APPLICATION.pathImages##folder#/#URL.id#/")) {
		DirectoryCreate("#APPLICATION.pathImages##folder#/#URL.id#/");
	}
	...
	if ( isNull( form.chunks ) ) {
		fileMove(
			form.file,
			"#APPLICATION.pathImages##folder#/#URL.id#/#form.name#" 
		);
	// We are executing a chunked upload.
	} else {
		if(NOT DirectoryExists("#APPLICATION.pathImages##folder#/chunks/#URL.id#/")) {
			DirectoryCreate("#APPLICATION.pathImages##folder#/chunks/#URL.id#/");
		}
		...
		upload = fileOpen( "#APPLICATION.pathImages##folder#/chunks/#URL.id#/#form.name#", "append" );
		...
		if ( form.chunk == ( form.chunks - 1 ) ) {
			fileMove(
				"#APPLICATION.pathImages##folder#/chunks/#URL.id#/#form.name#",
				"#APPLICATION.pathImages##folder#/#URL.id#/#form.name#"
			);
		}
		DirectoryDelete("#APPLICATION.pathImages##folder#/chunks/#URL.id#/");
	}
