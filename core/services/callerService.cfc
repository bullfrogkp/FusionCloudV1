...
<CFCOMPONENT>
	...
	<cffunction name="getNVPResponse" access="public" returntype="struct">
		...
		<cfloop from="1" to="#len(nvpString)#" index="LOCAL.i">
			<cfset lennvp = lennvp + 1>
			<cfset index = 1>
			<cfif #Mid(nvpString, LOCAL.i, 1)# is not "&">
			<cfset tempVal = tempVal & #Mid(nvpString, LOCAL.i, 1)#>
			</cfif>

			<cfif #Mid(nvpString, LOCAL.i, 1)# is "&" OR "#lennvp#" is "#len(nvpString)#">
		...
		<cfloop from="1" to="#ArrayLen(nvpArray)#" index="LOCAL.i">
			<cfset currentValue = #nvpArray[LOCAL.i]#>
			
			<cfloop from="1" to= "#len(currentValue)#" index="LOCAL.i">
				<cfif #Mid(currentValue, LOCAL.i, 1)# is "=">
				<cfbreak>
				<cfelse>	
				<cfset tempValue = tempValue & #Mid(currentValue, LOCAL.i, 1)#>
			...
			<cfset key = "">
			<cfloop from="1" to="#ArrayLen(nvpArray)#" index="LOCAL.i">
			<cfset vals  = #nvpArray[LOCAL.i]# >
			<cfset key = #keyArray[LOCAL.i]#>
			<cfset value = #RemoveChars(vals ,1, (len(key) +1))#>
			<cfset valueArray[LOCAL.i] = value>
